...
void parse_data_fun(std_msgs::String msg)
{
	...
	if(msg_info.target_id=='A'||msg_info.target_id==MY_ID)
	{
		...
		case 0:// msg id is 0
			...
			body_rate.x = attitude_parse.q.w;
			body_rate.y = attitude_parse.q.x;
			body_rate.z = attitude_parse.q.y;
			ROS_INFO_STREAM("body_rate x: "<<body_rate.x<<" y:"<<body_rate.y<<" z:"<<body_rate.z);
...
int main(int argc,char** argv)
{
	...
	ros::Publisher attitude_body_rate_pub = nh.advertise<geometry_msgs::Point>("Attitude_body_rate",10);
	...
	while (ros::ok())
	{
		...
		if(q.w!=0 && q.x!=0 && q.y!=0 && q.z!=0 && thrust.data!=0)
		{
			if(thrust.data>0.000001 && thrust.data < 100)
			{
				...
				attitude_body_rate_pub.publish(body_rate);
			}
